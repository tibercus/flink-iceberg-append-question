...
public class FlinkSinkAppend {
    ...
    public static void main(String[] args) throws Exception {
        ...
        tEnv.executeSql("DROP TABLE IF EXISTS catalog.db.flink_sink_append");
        tEnv.executeSql(
        ...
        FlinkSink.forRow(source, tableSchemaBuilder.build())
                .tableLoader(TableLoader.fromHadoopTable(catalogPath + "/db/flink_sink_append"))
                .append();

        env.execute();

        System.out.println("select count(*)");
        try (CloseableIterator<Row> result = tEnv.executeSql("SELECT count(*) FROM catalog.db.flink_sink_append").collect()) {
            // expected output: 2
            // actual output: 1
        ...
        try (CloseableIterator<Row> result = tEnv.executeSql("SELECT * FROM catalog.db.flink_sink_append").collect()) {
            // expected output:
            // 1, value 1
            // 1, value 2
